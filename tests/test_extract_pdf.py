...
parse_balance_block = extract_pdf.parse_balance_block
...
|----------------------------|----------------------------|
| Previous Balance           | $666.66                    |
| Payments & Credits         | $0.00                      |
| Purchases & Other Charges  | $77.33                     |
| Cash Advances              | $0.00                      |
| Interest                   | $0.00                      |
| Fees                       | $0.00                      |
| Sub-total                  | $743.99                    |
...
# -- Parse Balance Block ---------------------
def test_parse_balance_block():
    blocks = extract_table_blocks(YEAR_TRANSITION_MD)
    balance = parse_balance_block(blocks)
    assert balance["Previous Balance"] == 666.66
    assert balance["NEW BALANCE"] == 743.99
    assert balance["Purchases & Other Charges"] == 77.33


def test_parse_balance_block_missing():
    blocks = ["| TRANSACTION DATE | POSTING DATE | AMOUNT($) |"]
    assert parse_balance_block(blocks) == {}
...
def test_process_pdf(mock_extract):
    ...
    assert df.iloc[0]["previous_balance"] == 100.00
    assert df.iloc[0]["new_balance"] == 132.33
